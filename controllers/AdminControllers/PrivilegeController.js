...
class PrivilegeController {
    ...
    async selectAllPrivilege(){
        //select the all the roles
        let roles = await this.RolesModel.selectAllRoles([], 'no', 'no', 'id', 'desc');

        //select all the user types
        let typeOfUser = await this.TypeOfUsers.selectAllTypeOfUsers([], 'no', 'no', 'id', 'desc');

        //select the all the privileges
        let privilege = await this.Priviledges.selectAllPrivileges([], 'no', 'no', 'id', 'desc');
                ...
                for(let e in roles){
                    ...
                    if(privilege.length > 0){
                        ...
                        for(let m in privilege){
                            ...
                            if(currentTypeOfUserId === typeOfUserUniqueId && currentRoleId === roleUniqueId){
                                let add = this.returnStatus(roleSummaryObject, typeOfUser, i, roles, e);
                                if(add === 'yes') {
                                    console.log('kjhjh')
                                ...
                                if(add === 'yes'){
                                    roleSummaryObject.push({
                                        type_of_user_unique_id:typeOfUser[i].unique_id, type_of_user: typeOfUser[i].type_of_user, role_unique_id:roles[e].unique_id, role:roles[e].role, status:'inactive'
                                    });
                                }
                            }
                        }
