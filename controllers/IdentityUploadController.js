...
class IdentityUploadController {
  ...
  async uploadIdCard(req, res) {
    try {
      ...
      if (
        userObject.id_upload_status ===
        this.AccountVerificationLevels.id_upload_pending
      ) {
        //unlink the file
      ...
      let response = await this.sendEmailANDPhoneMessageForSuccessfulIDUpload(
        userObject
      );
      ...
      this.responseObject.setMessage(
        "You have successfully uploaded your ID, please wait while we review the document"
      );
